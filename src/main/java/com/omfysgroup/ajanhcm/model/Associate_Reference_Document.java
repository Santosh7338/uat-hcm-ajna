...
@Entity
@Table(name="ASSOCIATE_REFERENCE_DOCS")
public class Associate_Reference_Document {
	@Id
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "associate_refdocs_seq")
	@SequenceGenerator(name = "associate_refdocs_seq", sequenceName = "ASSOCIATE_REFERENCE_DOCS_SEQ", allocationSize = 1)
	
	@Column(name = "ASSO_REF_DOC_ID")
	private int asso_ref_doc_id;
	...
	public int getAsso_ref_doc_id() {
		return asso_ref_doc_id;
	}

	public void setAsso_ref_doc_id(int asso_ref_doc_id) {
		this.asso_ref_doc_id = asso_ref_doc_id;
	}
